...
class GreenUnet160(Model):
    ...
    def __init__(self):
        ...
        ### Decoder     
        self.upconv3 = Conv2DTranspose(128, kernel_size=3, strides=2, padding = 'same', activation=lrelu)
        ...
        self.upconv2= Conv2DTranspose(64, kernel_size=3, strides=2, padding = 'same', activation=lrelu)
        ...
        self.upconv1= Conv2DTranspose(32, kernel_size=3, strides=2, padding = 'same', activation=lrelu)
...
class GreenUnet160c(Model):
    '''
    This model takes NIR image as input, and generate green image as output.
    Use Unet-like architecture with input size 160 * 160.
    Use UpSampling2D + Conv2D as up-conv layer.
    Discard the use of dropout layers.
    Use batch normalization to improve learning performance.
    Depth = 5
    '''
    
    def __init__(self):
        super(GreenUnet160c, self).__init__()
        ...
        self.BN4u = BatchNormalization()
        self.BN3u = BatchNormalization()
        self.BN2u = BatchNormalization()
        self.BN1u = BatchNormalization()
    ...
    def call(self, inputs):
        x = self.conv1_1(inputs)
        shortcut1 = self.conv1_2(x)
        x = self.BN1(shortcut1)
        x = MaxPooling2D(2)(x)
        x = self.conv2_1(x)
        shortcut2 = self.conv2_2(x)
        x = self.BN2(shortcut2)
        x = MaxPooling2D(2)(x)
        x = self.conv3_1(x)
        shortcut3 = self.conv3_2(x)
        x = self.BN3(shortcut3)
        x = MaxPooling2D(2)(x)
        x = self.conv4_1(x)
        shortcut4 = self.conv4_2(x)
        x = self.BN4(shortcut4)
        x = MaxPooling2D(2)(x)
        x = self.conv5_1(x)
        x = self.conv5_2(x)
